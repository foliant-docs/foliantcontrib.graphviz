...
`fix_svg_size`
:   If `true` —  If True, replaces fixed width/height values (e.g., "500px", "300") with "100%" to make SVG responsive and scale to container size. Default: `true`

`compress_svg`
:   If `true` — removes whitespace and line breaks between tags to create a compact, single-line SVG. This helps prevent SVG from breaking HTML layout and reduces file size. Default: `true`
